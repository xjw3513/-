...
def get(html):
    reg = re.compile(r'class="e">.*? <span title="(.*?)">.*? <span class="time">(.*?).*?<span class="sal">(.*?)</span><span class="d at">(.*?)|.*?<a .*?target="_blank" title="(.*?)"',re.S)#匹配换行符
...
headData = ['招聘职位','日期','薪资','地址','公司']#表头部信息
